...
const HelpDialog = ({post,setDialog}) => {
  ...
  const DeletPost = async () => {
    setLoading(true);
    try{
      const response = await fetch(`https://notinsta-backend.vercel.app/api/v1/post/delete/${post._id}`,{method: 'POST', credentials: 'include'});
  ...
  const SavePost = async () => {
    try {
      const response = await fetch(`https://notinsta-backend.vercel.app/api/v1/post/${post._id}/save`)
