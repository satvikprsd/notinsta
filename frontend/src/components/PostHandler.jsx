...
export const handleLike = async (user,profile,post,posts,isLiked,setIsLiked,setCurLikes,dispatch) => {
    console.log(post)
    try{
        const response = await fetch(`https://notinsta-gr7b.onrender.com/api/v1/post/${post._id}/likeordislike`, {credentials: 'include'});
...
export const handleNewComment = async (post,profile,posts,comments,setComments,commenttext,setCommenttext,dispatch,setCurComments) => {
    try{
        const response = await fetch(`https://notinsta-gr7b.onrender.com/api/v1/post/${post._id}/newcomment`, {
            method: 'POST',
            credentials: 'include',
            headers: {
